import 'react-native-get-random-values'; // <--- ESTO ES LO QUE CORRIGE EL ERROR
import React, { useEffect, useState } from 'react';
import { StatusBar, StyleSheet, LogBox, View, Text, Image } from 'react-native';
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setProgress(50);
        await delay(500);
        //await initializeTensorFlow(); // TF se inicializa aquí, una sola vez.
        setProgress(80);
        await delay(500);
        api.get('/').catch(() => {});
        setProgress(100);
