...
router.post('/register-facial', upload.single('imageFile'), async (req, res) => {
  let createdAuthUser = null; // Guardará el usuario de Auth para eliminarlo si falla Firestore
  ...
  try {
    ...
    // ROLLBACK: Si el usuario se alcanzó a crear en Auth pero falló algo después, lo eliminamos
    if (createdAuthUser) {
      try {
